            ...
            while "1/" in cadena:
                ...
                match option[0]:
                    ...
                    case "start":
                        ...
                        component_enabled = conexion.configuradorst80() 
                            ...
                            try:
                                ...
                                while not heatsink_scanned:
                                    ...
                                    if len(name_piece) > 27:
                                        ...
                                        url_api_padre_raw = url_api_unit.replace("serialnumber", name_piece)
                                ...
                                if component_enabled == '1':                                
                                    ...
                                    while not comp_scanned:
                                        ...
                                        if len(scanned_component) > 13:
                                            ...
                                            if str(json_comp.get("success")).lower() not in ("true", "1"):
                                                ...
                                                safe_insert(f"Component Rejected: {msg_err_c}", "red")
                                    ...
                                    interlocking_json_api = interlocking_json.interlocking_component_station_80(
                                        SERIAL_PADRE_GLOBAL,      
                                        PART_NUMBER,       
                                        component_sn 
                                    )
                                ...
                                interlocking_json_api = interlocking_json.interlocking_station_80(
                                    SERIAL_PADRE_GLOBAL,      
                                    PART_NUMBER, 
                                )
                                ...
                                logging.info(f"Response Interlocking JSON: {json.dumps(data_interlocking, indent=4)}")
                        ...
                        # BLOQUE NUEVO PARA PLC
                            ...
                            if component_enabled != '1':
                                safe_insert(f" ❌ Check the settings, because the component is not enabled.", "red")
                                conn.send("FAILED".encode('UTF-8'))
                                break
                            name_piece = str(option[1]).strip()
                            scanned_component = str(option[2]).strip()
                            
                            safe_insert(f"Validating Parent & Component...", "orange")
                            green_label.configure(image=image_green_full)
                            red_label.configure(image=image_red)
                            ...
                            try:
                                ...
                                data_node_padre = json_padre.get("data", {})
                                if isinstance(data_node_padre, list):
                                    data_node_padre = data_node_padre[0]

                                part_number_extraido_unit_component = data_node_padre.get("part_number", "")

                                url_comp_base = url_api_unit.replace("serialnumber", scanned_component)
                                ...
                                if str(json_comp.get("success")).lower() not in ("true", "1"):
                                    safe_insert(f"❌ [UNIT COMPONENT REJECTED]\n{json.dumps(json_comp, indent=2)}", "red")
                                    conn.send("FAILED".encode('UTF-8'))
                                    break
                            ...
                            if component_enabled == '1':
                                safe_insert(f" ❌ Check the settings, because the component is enabled.", "red")
                                conn.send("FAILED".encode('UTF-8'))
                                break
                            name_piece = str(option[1]).strip()

                            safe_insert(f"Validating Parent...", "orange")
                            green_label.configure(image=image_green_full)
                            red_label.configure(image=image_red)
                            ...
                            try:
                                ...
                                if response_padre.status_code != 200:
                                    safe_insert(f"API UNIT Parent {response_padre.status_code}", "red")
                                    conn.send(f"FAILED".encode('UTF-8'))
                                    break
                        ...
                        if len(option) == 6 and option[-1] == '1/':
                            ...
                            if duration == "PASSED":
                                try:
                                    ...
                                    if component_enabled == '1':
                                        traceability_payload = traceability_json.traceability_component_station_80(
                                            option[4],
                                            ""
                                        )
                                    else:
                                        traceability_payload = traceability_json.traceability_station_80(
                                            option[4],
                                            ""
                                        )
                                    ...
                                    pantalla_end = (
                                        ...
                                        f"\nTraceability Data Dispatched & Saved Successfully"
                                        f"\n\n[TRACEABILITY]:\n{json.dumps(traceability_payload, indent=4, ensure_ascii=False)}\n\n"
