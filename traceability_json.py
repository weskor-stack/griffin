...
def traceability_station_60(serial_padre, defect_code_default=""):
    ...
    # Dar el formato deseado
    fecha_formateada = fecha_dt.strftime("%d-%m-%Y %H:%M:%S %p")
    ...
    for row in all_test_rows:
        try:
            ...
            unidad = str(row[5]) if row[5] is not None else ""
            status_step = str(row[6]).upper() if row[6] is not None else "PASSED"
            name_step = str(row[10]) if row[10] is not None else "Measurement"
