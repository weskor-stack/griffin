...
def interlocking_station_20(parent_serial_number, heater_serial_number, plc_value, plc_defect_code):
    
    # 1. Extracción de datos desde conexion.py
    config = conexion.configurador()
    if not config or config == "FAILED":
        return "Error: No se encontró configuración."
        
    machine_id = config[0]         # Posición 2 (machine_id)
    process_name = config[1]       # Posición 3 (process_name)
    operator = config[2]           # Posición 7 (operator)
    program_name = config[4]       # Posición 1 de tu mapa / program_name_version de la DB
    product = config[8]            # Posición 9 (product)
    ...
    estructura_json = {
        ...
        "commands": [], # Lo iniciamos vacío y lo llenamos condicionalmente
        "test_steps": {
...
if __name__ == "__main__":
    resultado_json = interlocking_station_20(
        parent_serial_number="P2170207-00-E:SE4A26127000245",
